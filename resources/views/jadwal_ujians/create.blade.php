...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="page-title mb-1">Tambah Jadwal Ujian</h3>
        <div class="text-muted">Tambahkan tanggal pelaksanaan berdasarkan tahap ujian.</div>
    </div>
...
<div class="card page-card">
    <div class="card-body">
        <form action="{{ route('jadwal-ujians.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="tahap_ujian_id" class="form-select" required>
                    ...
                    @foreach($tahapUjians as $tahap)
                        <option value="{{ $tahap->id }}" {{ old('tahap_ujian_id') == $tahap->id ? 'selected' : '' }}>
                            {{ $tahap->nama }}
                        </option>
                    @endforeach
                </select>
